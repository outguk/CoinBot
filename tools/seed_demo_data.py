...
커버 시나리오
  ...
  - BUY/SELL 신호, exit_reason 5종(단순+조합): 파이차트 전 케이스 커버
  - is_partial=1 케이스: 부분청산 메트릭, 반투명 SELL 마커
  - BUY RSI 20~30(≤oversold 30), SELL RSI 70~85(≥overbought 70) / 가격청산 30~69: C++ 전략 정렬
  - stop=10%, target=15%: C++ Params::stopLossPct / profitTargetPct 정렬
  - intrabar 청산: 5% 확률 급변동 캔들(spread 8~18%) → stop 10% / target 15% 봉 내 도달 → intrabar=True 검증
"""
...
# vol: 15분봉 1-bar 변동성 (σ).
# min_volatility=0.004(C++) 기준: rolling(20) stdev ≈ vol이므로
# vol이 0.004와 같으면 임계값 통과가 ~50%에 그쳐 진입이 거의 안 생김.
# vol=0.008로 설정하면 rolling stdev ≈ 0.008 >> 0.004 → 항상 통과,
# RSI(14)도 극단값(≤30) 도달 빈도 증가 → 백테스트 진입 안정적으로 발생.
_MARKETS = {
    "KRW-BTC": {"start_price": 80_000_000, "vol": 0.008, "n_trades": 20},
    "KRW-ETH": {"start_price":  4_000_000, "vol": 0.008, "n_trades": 18},
}
...
def generate_candles(market: str, start_price: float, vol: float, unit: int) -> list[dict]:
    ...
    for _ in range(n_bars):
        ...
        # 5% 확률로 급변동 캔들: stop 10% / target 15% 봉 내 도달 → intrabar=True 검증용
        if rng.random() < 0.05:
            spread = close * rng.uniform(0.08, 0.18)
        else:
            spread = close * rng.uniform(0.005, 0.04)
...
def generate_trades(
    ...
    for i in range(n_trades):
        ...
        # 손절/익절 가격: C++ Params::stopLossPct=10, profitTargetPct=15
        stop_pct     = 0.10
        target_pct   = 0.15
        ...
        # ── signals ──────────────────────────────────────────────────────────
        # 변동성: C++ minVolatility=0.004 이상 (진입 조건 통과한 거래)
        volatility     = round(rng.uniform(0.005, 0.04), 4)
        # 추세 강도: C++ maxTrendStrength=0.04 이하 (진입 조건 통과한 거래)
        trend_strength = round(rng.uniform(0.002, 0.035), 4)

        # BUY RSI: C++ oversold=30 이하에서 진입
        buy_rsi = round(rng.uniform(20, 30), 1)
        # SELL RSI: RSI 기반 청산은 overbought=70 이상, 가격 기반 청산은 미달 범위
        rsi_exit = exit_reason in ("exit_rsi_overbought", "exit_target_rsi_overbought",
                                   "exit_stop_rsi_overbought")
        sell_rsi = round(rng.uniform(70, 85) if rsi_exit else rng.uniform(30, 69), 1)
