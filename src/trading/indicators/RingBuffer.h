	...
	*/
	...
	private:
		...
		*/
		...
		std::vector<T> buf_{};	// 실제 데이터를 담는 저장소
		std::size_t cap_{ 0 };	// 이 버퍼가 최대 몇 개까지 가능한지에 대한 논리적 규칙

		// head_는 “oldest”의 물리 인덱스를 가리킨다( size_ > 0일 때 의미 있음 )
		std::size_t head_{ 0 };

		// 현재 유효한 데이터 개수 (0..cap_)
		std::size_t size_{ 0 }; // 현재 유효한 데이터의 개수에 대한 논리(0이면 실제 데이터는 있지만 사용 못함)
	};
}