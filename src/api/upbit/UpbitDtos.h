...
// Upbit JSON 데이터를 그대로 받는 구조체(Dto) 정의

namespace api::upbit
{
	
	// 업비트 마켓(종목) 정보 DTO
	struct UpbitMarketEventDto 
	{
		struct UpbitMarketCautionDto
		{
		...
		std::optional<UpbitMarketCautionDto> caution; // 주의 종목 정보 (없을 수도 있으니 optional 권장)
	};
	struct UpbitMarketDto
	{
		std::string			market;			// 마켓 코드 (ex: KRW-BTC)
		std::string			korean_name;	// 한글명
		std::string			english_name;	// 영문명

		std::optional<UpbitMarketEventDto> market_event; // 마켓 경고 정보 (없을 수도 있으니 optional 권장)
	};

	/*
	* 현재는 직접 종목을 골라서 시세 정보를 받는 페어 단위의 Ticker DTO만 정의
	* 추후 실시간을 거래 종목을 고르는 경우 마켓 단위의 조회 DTO도 정의 필요
	*/

	// 업비트 페어 단위 시세 정보 DTO (종목은 내가 고름)
	struct UpbitTickerDto
	{
		// 기본 정보
		std::string			market;					// 마켓 코드 (ex: KRW-BTC)
	...
	struct UpbitOrderbookDto
	{
		// 기본 정보
		std::string		market;			// 마켓 코드 (ex: KRW-BTC)
		int				timestamp;		// 조회 요청 시각의 타임스탬프(ms)
	...
	struct UpbitOrderbookPolicyDto
	{
		std::string		market;				// 마켓 코드 (ex: KRW-BTC)
