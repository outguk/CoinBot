...
namespace api::ws {
    ...
    void UpbitWebSocketClient::runReadLoop() 
    {
        ...
        while (!stop_.load(std::memory_order_relaxed))
        {
            // 1) 커맨드 처리 (한 번에 여러 개 처리)
            {
                ...
                // 로컬 큐를 만들고 cmd_q 내용을 빠르게 swap해 바로 처리할 수 있게 설정
                ...
                for (auto& c : local)
                {
                    if (std::holds_alternative<CmdClose>(c)) // close면
                    {
                        stop_.store(true, std::memory_order_relaxed);  // true로 변경
                        break; // run 종료
                    }

                    if (auto* cc = std::get_if<CmdConnect>(&c)) // 연결 신호라면
                    {
                        connectImpl(cc->host, cc->port, cc->target, cc->bearer_jwt); // 연결 시도
                        if (ws_ && ws_->is_open()) // 연결에 성공하면
                            resubscribeAll();      // 구독을 다시한다
                        continue;
                    }

                    if (auto* sc = std::get_if<CmdSubCandles>(&c)) // 캔들 구독 신호라면
                    {
                        const std::string ticket = makeTicket();   // 요청 구분자를 생성
                        const std::string frame = buildCandleSubJsonFrame( // 캔들 구독 요청 포맷을 생성
                            ticket, sc->type, sc->markets,
                            sc->is_only_snapshot, sc->is_only_realtime, sc->format);

                        last_sub_frames_[sc->type] = frame; // 마지막 정보를 저장할 프레임 생성
                        (void)sendTextFrame(frame); // 구독 요청을 보낸다
                        std::cout << "[WS] Candle Subscribe sent: " << sc->type << "\n";
                        continue;
                    }

                    if (auto* sm = std::get_if<CmdSubMyOrder>(&c))  // 내 주문 체결 구독 신호라면
                    {
            ...
            if (on_msg_)
                on_msg_(std::string_view(msg)); // 저장된 람다 함수 호출
        }
    ...
    */
    std::string UpbitWebSocketClient::buildCandleSubJsonFrame(
        ...
        return root.dump(); // json 형식 string 반환
    }
