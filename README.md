...
### 6. Recovery and Operational Resilience
...
### 7. SQLite + Streamlit + Backtest Pipeline
...
stateDiagram-v2
    [*] --> Flat
    Flat --> PendingEntry : RSI ≤ oversold\n+ 시장 적합성 OK
    PendingEntry --> InPosition : PositionEffect::Opened\n진입가 확정, 손절/익절 설정
    PendingEntry --> Flat : PositionEffect::None\n체결 없이 취소
    InPosition --> PendingExit : 확정봉 RSI ≥ overbought\nOR 손절/익절 도달
    InPosition --> PendingExit : 미확정 틱 손절/익절 도달\n(Intrabar Exit)
    PendingExit --> Flat : PositionEffect::Closed
    PendingExit --> InPosition : PositionEffect::None\n체결 없이 취소
```
