...
## E. myOrder 최종 체결 정산 누락 가능성(현행 Known Limitation)
현행 구조 요약:
- onMyTrade: AccountManager 즉시 정산(체결 단건 기준)
- onOrderSnapshot: OrderStore 누적 스냅샷 동기화

리스크 시나리오:
- 부분 체결 진행 중 마지막 체결(trade)이 지연/유실되거나,
- done/cancel 스냅샷이 먼저 처리되는 역순 케이스에서,
- AccountManager가 마지막 체결분을 반영하지 못할 수 있음.

빈도(운영 체감) 판단:
- 정상 네트워크/단일 연결에서는 낮음(드문 편).
- 재연결 구간, 패킷 손실, 서버/클라이언트 지연 급증 구간에서는 확률 상승.
- 즉 "항상 발생"은 아니지만 24시간 장기 운용에서는 누적적으로 재현 가능성이 있는 클래스.

중요도 판단:
- 중요도: 높음(High)
- 이유: 발생 빈도는 낮~중이지만, 1회 발생 시 자산 정합성(AccountManager vs 실제 체결)이 틀어질 수 있음.
- 영향: 전략 상태 복구 지연, 잔고 추정 오차, 후속 주문 판단 오염 가능.

현재 의사결정(이번 단계):
- 우선 코드 수정은 보류하고, 본 리스크를 명시적으로 관리한다.
- 운영 중 탐지 신호를 기록/모니터링한다.

권장 모니터링 지표(수정 전 운영 가드):
- done/cancel 직후 trade 후행 수신 비율
- 주문 단위 누적값 불일치(OrderStore.executed_funds/paid_fee vs 체결 이벤트 합)
- 주기적 syncWithAccount 이후 보정 발생 횟수

후속 개선 후보(차기):
- 터미널 스냅샷 시 누락 delta 보정 정산
- 이벤트 순서 내구성 강화(역순/유실 전제)
